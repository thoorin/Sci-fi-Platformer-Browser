    ...
    if ( event.phase == "began" ) then    
        if ( M.isCollided("player")) then
            ...
            then   
                ...
                if ((player.y - block.y) < -46.9) then game.setCanJump(true) end
                if (playerContactedByMoving and (player.y - block.y) < -44) then game.setCanJump(true) end

                if (math.abs(obj1.y - obj2.y) < 60 and game.getStopped()==false and game.getCanJump()==false and game.getGame()==true and M.findObject("blockMoving")==false) then 
                    game.stopping() 
